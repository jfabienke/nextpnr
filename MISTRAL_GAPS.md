...
| **G3** | HPS hard IP: only `mpu_general_purpose` had a bel | **lwh2f BEL BUILT; silicon test BLOCKED on board network state.** The bel (globals.cc `create_hps_lwh2f`) instantiates, packs, places and ROUTES a minimal single-handshake design. Silicon unverified: the board dropped to a 169.254 link-local address (off its subnet), unreachable until it re-acquires DHCP. Also found: the router cannot legalise a WIDE bus escaping the HPS corner (many parallel signals deadlock at cong=2) — same LUT-permutation gap as G2, so the first working core should keep the h2f data path narrow or await the router fix. Then f2sdram |
