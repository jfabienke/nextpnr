...
| **G6** | bidirectional IO, then the SDRAM controller path | **slices 1–3 SOLVED @ 50 MHz** — tristate pads + full 32 MB MemTest, 0 errors. **135 MHz (VRAM rate) does NOT work with the minimal controller** — 255 errors from addr 0, and a phase sweep (0–5556 ps, proper reboots) does not fix it. The DQ read path (SDRAM clk-to-out ~5.5 ns + trace + fabric routing) exceeds the 7.4 ns period, so 135 MHz needs an **IO INPUT REGISTER** at the pad — the unimplemented IO-register feature. So VRAM-speed SDRAM depends on IO-register packing (or a ported performance controller that uses it) |
...
|---|---|---|
| IO registers (`FAST_*_REGISTER`) | **not packed** — now a BLOCKER for G6 @ 135 MHz: the DQ read capture needs an IO input register to close timing at speed. Ground truth puts them in the DQS16 block | `bitstream.cc` |
